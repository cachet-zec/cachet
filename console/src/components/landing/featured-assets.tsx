...
/**
 * The latest registry entries whose name is actually attested, presented as
 * ledger lines.
 *
 ...
 * does. Nothing is withheld - the console lists everything, and the link
 * beside this block goes straight there.
 */
/** Rows shown on the landing; also the server-side fetch limit. */
...
export function FeaturedAssets() {
  ...
  const { data, error, isPending } = useQuery({
    queryKey: ["assets", "featured", "resolved"],
    queryFn: async () => {
      const { data, error } = await api.GET("/api/v1/assets", {
        params: { query: { limit: FEATURED_COUNT, resolved: true } },
      });
  ...
  const entries = data.slice(0, FEATURED_COUNT);
  if (entries.length === 0) {
    return (
      <p className="border-y border-white/[0.07] py-8 text-sm text-neutral-500">
