...
// CACHET PATCH (see vendor/librustzcash/README.md): the Orchard proving
// keys are deterministic and expensive to build (tens of seconds in
// single-threaded wasm), yet upstream rebuilds them inside every
// `build()` call. Memoize them per process; `prepare_*` lets callers
// warm the cache off the critical path (e.g. while a user fills a form).
#[cfg(feature = "circuits")]
fn orchard_vanilla_proving_key() -> &'static orchard::circuit::ProvingKey {
...
/// CACHET PATCH: build (and cache) the OrchardZSA proving key now, so a
/// later `Builder::build()` on this process skips the expensive step.
#[cfg(all(feature = "circuits", zcash_unstable = "nu7"))]
pub fn prepare_orchard_zsa_proving_key() {
    let _ = orchard_zsa_proving_key();
}
